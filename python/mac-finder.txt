...
#  --------------------------------
def setMACTable(line, ttiuser, ttipass, macmac):
	commands = 'show mac address address ' + macmac
...
def setData(line5):
	...
	intf = 'interface'
	#
	with open('files/' + line5 + '-config-final.txt', 'w') as final_data:
...
with open('files/ios-iplist.txt', 'r') as ccthost:
	for line in ccthost:																											
		...
		#
		mac_int = setMACTable(line, ttiuser, ttipass, macadd1)
		trunk_out = setTrunk(line, ttiuser, ttipass)
		trunk_port = trunk_out.split()
		cnt = 0
		while cnt <= (len(trunk_port) - 1):
			if mac_int == trunk_port[cnt]:
				acnt = cnt - 2
